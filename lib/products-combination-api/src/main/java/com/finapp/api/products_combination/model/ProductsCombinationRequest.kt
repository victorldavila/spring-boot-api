...
data class ProductsCombinationRequest(
    ...
    @field:NotBlank(groups = [ OnCreate::class ], message = "Product Combination type can not be blanked")
    @JsonProperty("type") val type: ProductsCombinationType?,
    @field:NotBlank(groups = [ OnCreate::class ], message = "Product Combination category can not be blanked")
