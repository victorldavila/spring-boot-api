...
class ProductRouter(
    ...
    fun productsRoutes(): RouterFunction<ServerResponse> =
        route(ApiRouter.apiGET("/v1/products/{userId}"), productHandler::getProductById)
        ...
        .andRoute(ApiRouter.apiPUT("/v1/products/{userId}"), productHandler::updateProduct)
            .filter(getProductUpdateAuthorizationFilter())
        .andRoute(ApiRouter.apiDELETE("/v1/products/{userId}"), productHandler::deleteProduct)
